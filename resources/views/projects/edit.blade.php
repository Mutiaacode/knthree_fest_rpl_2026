    ...
    <form action="{{ route('projects.update', $project->id) }}" method="POST" enctype="multipart/form-data">
        ...
        <div class="mb-4">
            ...
            <select name="kelas" required class="w-full border-gray-300 rounded-md shadow-sm focus:border-blue-500 focus:ring focus:ring-blue-200 p-2 border">
                ...
                <option value="X RPL A" {{ old('kelas', $project->kelas) == 'X RPL A' ? 'selected' : '' }}>X RPL A</option>
                <option value="X RPL B" {{ old('kelas', $project->kelas) == 'X RPL B' ? 'selected' : '' }}>X RPL B</option>
                <option value="XI RPL A" {{ old('kelas', $project->kelas) == 'XI RPL A' ? 'selected' : '' }}>XI RPL A</option>
                <option value="XI RPL B" {{ old('kelas', $project->kelas) == 'XI RPL B' ? 'selected' : '' }}>XI RPL B</option>
                <option value="XII RPL A" {{ old('kelas', $project->kelas) == 'XII RPL A' ? 'selected' : '' }}>XII RPL A</option>
                <option value="XII RPL B" {{ old('kelas', $project->kelas) == 'XII RPL B' ? 'selected' : '' }}>XII RPL B</option>
            </select>
        </div>
